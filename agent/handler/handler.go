...
func buildNiobeInstruction(waitress *store.Waitress, toolExecutor *tools.LocalNiobeTools, tableFromURL string) string {
	...
	parts = append(parts,
		...
		"Use this flow for action requests: confirm the details, wait for the user to confirm (yes, yeah, no problem, go ahead, sure, etc.). As soon as they confirm, call the tool with order_details set to the order summary. After the tool returns success, say once that the request is done and ask if they need anything else—do not repeat the same 'order placed' or 'done' message.",
